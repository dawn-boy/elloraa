...
**Tech Stack:** Python managed via the `uv` environment manager. All tooling and dependencies should be strictly organized using `uv`.

**Current Priority:** The immediate development focus is strictly on building out the pipeline **up to the Voice Stage (Stage 1)**. This includes:
1.  **Manifest Management:** Defining the Pydantic schema for the Beats Manifest and managing state.
2.  **Stage 0 (Director Pass):** LLM integration to generate the structured manifest.
3.  **Stage 1 (Voice Generation):** Implementing the IndexTTS2 wrapper (Note: documentation for IndexTTS2 is available in the `documentations/` folder).

Downstream stages (Image, Music, Compositing) will follow once the voice stage is fully working and stable.

---

## Project Structure & State Management

**STATUS.md:** Located at the project root, `STATUS.md` acts as the active memory and handoff document between AI sessions. It tracks completed milestones, architectural decisions, and explicitly defines the immediate next steps. Always consult `STATUS.md` before beginning work to understand the current context and where to pick up.

**Directory Intentions:**
The project structure strictly separates code, immutable inputs, and generated outputs to maintain a clean environment:
- `src/` - All pipeline logic, Pydantic schemas (`manifest.py`), orchestration, and individual stage implementations (`stages/`).
- `assets/` - Immutable inputs. Reference voice files (`assets/voices/`), raw scripts (`assets/scripts/`), and eventually static elements like fonts.
- `outputs/` - Sandboxed directory for all pipeline generations. Evolving manifests (`outputs/manifests/`), generated audio, images, and final videos go here.
- `documentations/` - Scraped or manually written documentation for local AI models (e.g., IndexTTS2) to ensure agents use accurate API calls without browsing the web.
...
IndexTTS2 is the primary voice model. It clones from a reference audio file and applies an emotion vector to control expressive tone. *(Note: Scraped documentation for IndexTTS2 is available under the `documentations/` folder.)*
